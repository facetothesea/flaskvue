...
def mat_getlist():
    ...
    result=[]
    for s in mats:
        result.append({'code':s.id,'name':s.name,'pcs':s.pcs,'state':s.state,'remark':s.remark,'lastdt':s.lastdt})
...
def sto_getlist():
    ...
    result=[]
    for s in stos:
        result.append({'allocation':s.allocation,'state':s.state,'remark':s.remark,'lastdt':s.lastdt})
    return jsonify(result)
...
def saf_getlist():
    ...
    result=[]
    for s in saf:
        result.append({'code':s.code,'name':s.name,'acount':s.acount,'remark':s.remark,'lastdt':s.lastdt})
    return jsonify(result)
...
def inv_getlist():
    ...
    """
    inv=db.session.query(Inventory.allocation,Inventory.code,Inventory.name,Inventory.inventory_acount,
    func.substr(Inventory.allocation,1,1),func.substr(Inventory.allocation,2,3)).all()
    result={}
    for i in inv:
        key1=i[4]
        key2=i[5]
