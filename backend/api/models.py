...
class Material(db.Model):
    ...
    lastdt = db.Column(db.DateTime, default=datetime.now)  #最后修改时间
...
class Storage(db.Model):
    ...
    lastdt = db.Column(db.DateTime, default=datetime.now)
    # inventory_id=db.Column(db.Integer,db.ForeignKey('inventory.id'))
    # state_describe=db.relationship('State',backref='state',lazy='dynamic')
    def __repr__(self):
...
class Inventory(db.Model):
    ...
    lastdt = db.Column(db.DateTime, default=datetime.now)  #修改时间
...
class Safety(db.Model):
    ...
    lastdt = db.Column(db.DateTime, default=datetime.now)
    def __repr__(self):
        return '< Safety {} {} > '.format(self.name,self.acount)
